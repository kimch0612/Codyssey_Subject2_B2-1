...
export에는 월 또는 시작일·종료일 범위가 필요합니다. 출력 파일이 이미 있으면 덮어쓰지 않습니다. import는 등록된 카테고리의 거래를 추가하며 처리 건수를 출력합니다. 정상 행은 저장하고 값 검증에 실패한 행은 건너뜁니다. 같은 CSV를 다시 가져오면 거래가 중복 등록될 수 있습니다.
